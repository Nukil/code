...
object WorkerServer extends App{
    ...
    def workerStart(): Unit = {
        val masterHost: String = LoadPropers.getProperties("akka").getProperty("akka.master.host", "127.0.0.1")
        val masterPort: Int = LoadPropers.getProperties("akka").getProperty("akka.master.port", "30055").toInt
